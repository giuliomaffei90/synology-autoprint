#!/usr/bin/env python3
"""Self-check: python3 test_autoprint.py (nessuna stampante necessaria)."""
import os
import struct
import sys
import tempfile
...
try:
    media_dimensions("carta_strana")
    assert False, "atteso errore su formato ignoto"
except PrintError:
    pass

# --- validazione JPEG ------------------------------------------------------
# JPEG minimo: SOI + APP0 + SOF0 (800x600) + EOI
jpg = (b"\xff\xd8"
...
with tempfile.TemporaryDirectory() as d:
    ...
    for name, blob in (("png.jpg", b"\x89PNG\r\n\x1a\n" + b"\x00" * 40),
        ...
        try:
            jpeg_info(bad)
            assert False, f"atteso errore su {name}"
        except PrintError:
            pass

# --- codifica/decodifica IPP ----------------------------------------------
...
# borderless: media-col con i quattro margini a zero al posto di 'media'
...
assert "media" not in bl_attrs, "borderless non deve inviare anche 'media'"
...
try:
    ...
    assert False, "attesa qualità non valida"
except PrintError:
    pass
